...
private Map getGasResult(value) {
    ...
    def descriptionText = value ? "${device.displayName} has detected GAS!" : "${device.displayName} is cleared!"
    log.debug descriptionText
	return [
			name			: 'smoke',
			value			: value ? 'detected' : 'cleared',
            isStateChange: true,
...
def refresh() {
    ...
    //zigbee.readAttribute(0x0009, 2) +	// not sure what this cluster do yet
